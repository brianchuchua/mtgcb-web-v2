...
const getActionDescription = (formData: MassEntryFormData, cardCount?: number): string => {
  ...
  if (mode === 'set') {
    return `Set all ${rarityLabel}${pagePhrase} to ${quantityReg} regular and ${quantityFoil} foil.`;
  } else {
    // Handle increment/decrement mode
    if (quantityReg === 0 && quantityFoil === 0) {
      return `No changes will be made to ${rarityLabel}${pagePhrase}.`;
    }
    ...
    return `${combinedAction} for all ${rarityLabel}${pagePhrase}.`;
  }
};

const MassEntryConfirmDialog: React.FC<MassEntryConfirmDialogProps> = ({
  open,
  onConfirm,
  onCancel,
  formData,
  ...
  const actionDescription = getActionDescription(formData, cardCount);
...
export default MassEntryConfirmDialog;
