...
export interface MassEntryFormData {
  mode: 'set' | 'increment';
...
interface MassEntryPanelProps {
  isOpen: boolean;
  onSubmit: (data: MassEntryFormData) => void;
  onCancel: () => void;
  isLoading?: boolean;
  cardCount: number;
}

const MassEntryPanel: React.FC<MassEntryPanelProps> = ({
  isOpen,
  onSubmit,
  onCancel,
  isLoading = false,
  ...
  const [formData, setFormData] = useState<MassEntryFormData>({
    mode: 'set',
    rarity: 'all',
    quantityReg: 0,
    quantityFoil: 0,
  ...
  const getActionDescription = (): string => {
    ...
    if (uiMode === 'set') {
      return `Set all ${rarityLabel} on this page to ${quantityReg} regular and ${quantityFoil} foil.`;
    } else if (uiMode === 'add') {
      if (quantityReg === 0 && quantityFoil === 0) {
        return `No changes will be made to ${rarityLabel} on this page.`;
      }
      ...
      return `${combinedAction} for all ${rarityLabel} on this page.`;
    } else {
      // Remove mode
      if (quantityReg === 0 && quantityFoil === 0) {
        return `No changes will be made to ${rarityLabel} on this page.`;
  ...
  const handleSubmit = () => {
    ...
    const apiFormData: MassEntryFormData = {
      ...formData,
    ...
    <Collapse in={isOpen}>
      <StyledPaper elevation={0}>
        <Typography variant="subtitle1" fontWeight="500" gutterBottom sx={{ textAlign: 'center' }}>
          Mass Update Up to {cardCount} Card{cardCount !== 1 ? 's' : ''} on This Page By
        </Typography>
